...
class IndexedServiceContainerSpec extends ObjectBehavior
{
    ...
    public function it_returns_custom_default_for_unexisting_parameter_if_provided()
    {
    ...
    public function it_knows_when_services_are_not_defined()
    {
        $this->has('some_service')->shouldReturn(false);
    }

    public function it_knows_when_services_are_defined($service)
    {
        $this->set('some_service', $service);
        $this->has('some_service')->shouldReturn(true);
    }

    public function it_returns_nothing_when_no_services_are_tagged()
    {
        $this->getByTag('some_tag')->shouldReturn([]);
    }

    public function it_returns_services_which_are_set_using_tags($service)
    {
        $obj = new \stdClass();
    ...
    public function it_returns_services_which_are_defined_using_tags()
    {
        $obj = new \stdClass();
        $this->define('some_service', function () use ($obj) {
            return $obj;
        }, ['some_tag']);
        $this->getByTag('some_tag')->shouldReturn([$obj]);
    }
    ...
    public function it_uses_new_definition_when_a_service_is_redefined()
    {
        $this->define('some_service', function () {
            return 1;
        });
...
//    }
    ...
    public function it_supports_custom_service_configurators()
    {

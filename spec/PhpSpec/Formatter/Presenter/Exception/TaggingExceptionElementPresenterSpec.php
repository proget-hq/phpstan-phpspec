...
{
    public function let(ExceptionTypePresenter $exceptionTypePresenter, ValuePresenter $valuePresenter)
    {
    ...
    public function it_should_tag_an_exception_thrown_message(
    ...
    public function it_should_present_a_tagged_code_line()
    {
        $this->presentCodeLine('3', 'foo')->shouldReturn('<lineno>3</lineno> <code>foo</code>');
    }

    public function it_should_present_a_tagged_highlighted_line()
    {
        $this->presentHighlight('foo')->shouldReturn('<hl>foo</hl>');
    }

    public function it_should_present_a_tagged_header_of_an_exception_trace()
    {
        $this->presentExceptionTraceHeader('foo')->shouldReturn('<trace>foo</trace>');
    }

    public function it_should_present_a_tagged_exception_trace_method(ValuePresenter $valuePresenter)
    {
        ...
        $this->presentExceptionTraceMethod('class', 'type', 'method', ['a', 'b'])->shouldReturn($result);
    }

    public function it_should_present_a_tagged_exception_trace_function(ValuePresenter $valuePresenter)
    {
        $valuePresenter->presentValue('a')->willReturn('zaz');
        $valuePresenter->presentValue('b')->willReturn('zbz');
        ...
        $this->presentExceptionTraceFunction('function', ['a', 'b'])->shouldReturn($result);
    }
}

...
class SuiteRunnerSpec extends ObjectBehavior
{
    public function let(
        ...
        $suite->getSpecifications()->willReturn([$spec1, $spec2]);
    }

    public function it_runs_all_specs_in_the_suite_through_the_specrunner($suite, $specRunner, $spec1, $spec2)
    {
    ...
    public function it_stops_running_subsequent_specs_when_a_spec_throws_a_StopOnFailureException($suite, $specRunner, $spec1, $spec2)
    {
    ...
    public function it_returns_a_successful_result_when_all_specs_in_suite_pass($suite, $specRunner, $spec1, $spec2)
    {
        $specRunner->run($spec1)->willReturn(ExampleEvent::PASSED);
        $specRunner->run($spec2)->willReturn(ExampleEvent::PASSED);
    ...
    public function it_returns_a_broken_result_when_one_spec_is_broken($suite, $specRunner, $spec1, $spec2)
    {
    ...
    public function it_returns_a_failed_result_when_one_spec_failed($suite, $specRunner, $spec1, $spec2)
    {
        $specRunner->run($spec1)->willReturn(ExampleEvent::FAILED);
    ...
    public function it_dispatches_events_before_and_after_the_suite($suite, $specRunner, $spec1, $spec2, $dispatcher)
    {
        $specRunner->run($spec1)->willReturn(ExampleEvent::PASSED);
        $specRunner->run($spec2)->willReturn(ExampleEvent::PASSED);
    ...
    public function it_dispatches_afterSuite_event_with_result_and_time($suite, $specRunner, $dispatcher)
    {
